...
function loadModel(modelName) {
  ...
  loadedObjects.push({
    ...
    originalColor: diffuseColor.slice(),
    ...
    currentMoveZ: 0,
    currentMoveY:
      "currentMoveY" in objectCharacteristics
        ? objectCharacteristics.currentMoveY
        : -minVertY,
    currentMoveX: 0,
    x: maxVertX - minVertX,
    y: maxVertY - minVertY,
  ...
  function drawObjects(shaderProgramNumber) {
    ...
    loadedObjects.forEach((todraw) => {
      ...
      if (todraw.currentScale != 1) {
        viewMatrix = utils.multiplyMatrices(
          ...
          utils.MakeTranslateMatrix(
            todraw.originX + todraw.currentMoveX,
            todraw.originY + todraw.currentMoveY,
            todraw.originZ + todraw.currentMoveZ
          )
        );
        viewMatrix = utils.multiplyMatrices(
          viewMatrix,
          utils.MakeScaleMatrix(todraw.currentScale)
        );
        viewMatrix = utils.multiplyMatrices(
          viewMatrix,
          utils.MakeTranslateMatrix(
      ...
      if (todraw.currentRotation) {
        ...
        viewMatrix = utils.multiplyMatrices(
          viewMatrix,
          utils.MakeRotateYMatrix(todraw.currentRotation)
        );
        viewMatrix = utils.multiplyMatrices(
          viewMatrix,
          utils.MakeTranslateMatrix(
      ...
      for (i = 0; i < todraw.sceneObjects; i++) {
        if (shaderProgramNumber == 0) {
          if (!todraw.isRoom) {
            gl.uniformMatrix4fv(
              rotationMatrixHandle[currentShader],
              gl.FALSE,
              rotationMatrix
            );
            gl.uniformMatrix4fv(
              translation[currentShader],
              gl.FALSE,
          ...
          if (todraw.isSelected) {
            gl.uniform1i(isSelectedLoc, true);
          } else {
            gl.uniform1i(isSelectedLoc, false);
          }

          if (todraw.isCollided) {
            gl.uniform1i(isCollidedLoc, true);
          } else {
            gl.uniform1i(isCollidedLoc, false);
          }

          gl.uniformMatrix4fv(
            matrixPositionHandle[currentShader],
            gl.FALSE,
            utils.transposeMatrix(projectionMatrix)
          );
          todraw.lightDirectionObj[i] = utils.multiplyMatrix3Vector3(
            utils.transposeMatrix3(
              utils.sub3x3from4x4(todraw.objectWorldMatrix[i])
            ),
            lightDirection
          );

          todraw.lightPositionObj[i] = utils.multiplyMatrix3Vector3(
            utils.invertMatrix3(
              utils.sub3x3from4x4(todraw.objectWorldMatrix[i])
            ),
            lightPosition
          ...
          todraw.observerPositionObj[i] = utils.multiplyMatrix3Vector3(
            utils.invertMatrix3(
              utils.sub3x3from4x4(todraw.objectWorldMatrix[i])
            ),
            eyeTemp
          ...
          gl.uniform1f(
            ambientLightInfluenceHandle[currentShader],
          ...
          if (
            todraw.nTexture[i] == true &&
            read_prop(todraw.diffuseTextureObj[i], "webGLTexture")
          ) {
          ...
          gl.uniform4f(
            lightColorHandle[currentShader],
            lightColor[0],
            lightColor[1],
            lightColor[2],
            lightColor[3]
          ...
          gl.uniform4f(
            materialDiffColorHandle[currentShader],
            todraw.diffuseColor[i][0],
          ...
          gl.uniform4f(
            materialSpecColorHandle[currentShader],
            todraw.specularColor[i][0],
            todraw.specularColor[i][1],
            todraw.specularColor[i][2],
            todraw.specularColor[i][3]
          );
          gl.uniform4f(
            ambientLightColorHandle[currentShader],
            ambientLightColor[0],
            ambientLightColor[1],
            ambientLightColor[2],
            ambientLightColor[3]
          ...
          gl.uniform4f(
            emissionColorHandle[currentShader],
            emitColor[0],
            emitColor[1],
            emitColor[2],
            emitColor[3]
          ...
          gl.uniform3f(
            lightDirectionHandle[currentShader],
            todraw.lightDirectionObj[i][0],
            todraw.lightDirectionObj[i][1],
            todraw.lightDirectionObj[i][2]
          );
          gl.uniform3f(
            lightPositionHandle[currentShader],
            todraw.lightPositionObj[i][0],
          ...
          gl.uniform1f(
            materialSpecPowerHandle[currentShader],
            objectSpecularPower
          );

          gl.enableVertexAttribArray(vertexPositionHandle[currentShader]);
          gl.vertexAttribPointer(
            vertexPositionHandle[currentShader],
            3,
            gl.FLOAT,
            gl.FALSE,
            4 * 8,
          ...
          gl.vertexAttribPointer(
            vertexNormalHandle[currentShader],
            3,
            gl.FLOAT,
            gl.FALSE,
            4 * 8,
...
function loadShaders() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    matrixPositionHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "wvpMatrix"
    );

    materialDiffColorHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "mDiffColor"
    );
    materialSpecColorHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "mSpecColor"
    );
    materialSpecPowerHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "mSpecPower"
    );

    textureFileHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "textureFile"
    );
    textureInfluenceHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "textureInfluence"
    );
    ambientLightInfluenceHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "ambientLightInfluence"
    );
    emitInfluenceHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "emitInfluence"
    );
    ambientLightColorHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "ambientLightColor"
    );
    emissionColorHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "emitColor"
    );

    eyePositionHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "eyePosition"
    );

    lightDirectionHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "lightDirection"
    );
    lightPositionHandle[i] = gl.getUniformLocation(
      shaderProgram[i],
      "lightPosition"
    );
