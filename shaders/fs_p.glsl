...
uniform bool isCollided; // Флаг коллизии (true - коллизия, false - нет коллизии)
uniform bool isSelected;
//uniform vec4 collisionColor; // Цвет при коллизии
...
void main() { 
	...
	else
	{
		...
		if(isSelected){
			gl_FragColor = min(ambLight + diffuse + specular + emit, vec4(0.0, 1.0, 0.0, 1.0)); 	
		}
		else{
			if(isCollided)
			{
				gl_FragColor = min(ambLight + diffuse + specular + emit, vec4(1.0, 0.0, 0.0, 1.0)); 		
			} else
			{
